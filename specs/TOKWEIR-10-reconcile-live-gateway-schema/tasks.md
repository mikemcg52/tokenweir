...
> **All boxes ticked.** Five review rounds ran against this list; where a review changed the
> plan, the change is recorded in `spec.md` (FR-011a/b, FR-015a, FR-016a/b, and the §Additions
> inventory) rather than by rewriting a task. Two tasks landed differently from how they were
> written and it is worth saying which: **T036** found the existing `psycopg` entry in
> `OPTIONAL_DRIVERS` already covers the new tests, so no entry was added — the check was the
> deliverable, not a change; and **T030**'s CLI grew a `--baseline-effective-from` whose `=` form
> is load-bearing (review 1, Med-2), which the task did not anticipate.

## Phase 1: Setup

- [x] **T001** Confirm the baseline is green before anything changes, using the authoritative
...
- [x] **T002** Build a second, throwaway venv with the `dev` extra so the real-Postgres suite
...
- [x] **T003** Create `src/tokenweir/reconcile.py` with the module docstring stating what this is
...
- [x] **T004** Implement `_snapshot(connection, schema)` (plan.md § 1): per-relation kind, and per
      ...
      story turns on.
- [x] **T005** Implement `reference_snapshot(connection)` (FR-004, FR-005, FR-006): unique scratch
...
- [x] **T006** [P] [US1] `tests/test_reconcile.py`: the `legacy_gateway_schema` fixture — the
...
- [x] **T007** [P] [US1] Assert `plan()` on a legacy database names the missing `schema_version`,
      the current-valued rate card and the view mismatch (US1 scenario 1).
- [x] **T008** [P] [US1] Assert `plan()` on a tokenweir-native database is empty (US1 scenario 2,
      FR-021).
- [x] **T009** [P] [US1] Assert `plan()` leaves the database unchanged — full snapshot before and
      after, plus "no `tokenweir_ref_*` schema survives" (US1 scenario 3, FR-002, FR-006).
- [x] **T010** [P] [US1] Assert an extra gateway column is reported and never proposed for removal;
      and that an extra `NOT NULL`-without-default column is `MANUAL` (US1 scenario 4, FR-009,
      FR-010).
- [x] **T011** [P] [US1] Assert `plan()` refuses an autocommit connection (FR-005).

### Implementation for User Story 1

- [x] **T012** [US1] Implement `plan(connection, *, baseline_effective_from=None)`: the generic
...
- [x] **T013** [US1] Implement the `schema_version` backfill resolution (FR-012): `ADD COLUMN …
...
- [x] **T014** [US1] Implement the rate-card restructure detection and resolution (FR-013), and the
      collision guard that makes it `MANUAL` (FR-014).
- [x] **T015** [US1] Implement the view comparison (FR-015): column set and order, not definition
      text; `DROP VIEW` + migration 005 on mismatch; `MANUAL` when `pg_depend` reports a dependent.
- [x] **T016** [US1] Make "no `gateway_usage` at all" and `UnknownAppliedVersionError` land as
...
- [x] **T017** [P] [US2] Row preservation: populate the legacy database, apply, assert every row
      present with every original column value unchanged, column-by-column (SC-002), and
      `schema_version = 1` throughout.
- [x] **T018** [P] [US2] Schema equivalence: after apply, the live snapshot equals a
      tokenweir-native snapshot, modulo the documented `id` exception and the reported extra
      columns (SC-001).
- [x] **T019** [P] [US2] The writer, both directions: `PostgresSource.write` raises
...
- [x] **T020** [P] [US2] The rollup: pre-existing rows price after reconciliation, to the figures
      the gateway's current-valued card would have produced (SC-004).
- [x] **T021** [P] [US2] Idempotence: a second run plans empty and executes nothing (FR-021).
- [x] **T022** [P] [US2] Atomicity: a statement that fails part-way leaves the database exactly as
      it was (US2 scenario 5, FR-018).
- [x] **T023** [P] [US2] `apply()` refuses a plan that no longer matches the live database
      (FR-020).

### Implementation for User Story 2

- [x] **T024** [US2] Implement `apply(connection, plan)`: re-derive, compare, refuse on `MANUAL`,
...
- [x] **T025** [P] [US3] Two rows for one model under different `pricing_mode`s: plan marks the
      restructure `MANUAL` and names the models (US3 scenario 1, FR-014).
- [x] **T026** [P] [US3] `--apply` with any `MANUAL` present changes **nothing at all** — snapshot
      before and after (US3 scenario 2, SC-005, FR-019).
- [x] **T027** [P] [US3] A rate-card restructure with no `--baseline-effective-from` refuses and
      explains what the date decides (US3 scenario 3, FR-013).
- [x] **T028** [P] [US3] A dependent view makes the rollup replacement `MANUAL` (US3 scenario 4,
      FR-015).

### Implementation for User Story 3

- [x] **T029** [US3] Wire the refusals into `apply()` and give each a message naming the object,
      the decision and the remedy. A refusal an operator cannot act on is a traceback with better
      manners.

## Phase 6: The command line

- [x] **T030** Add the `reconcile` subcommand to `src/tokenweir/migrations/__main__.py` (FR-022,
...
- [x] **T031** [P] Extend `tests/test_migrations_cli.py`: plan-by-default changes nothing, `--apply`
...
- [x] **T032** FR-025 — `_check_applied`'s adopted-versions warning names the reconciler. It is the
      one moment the library knows it is looking at somebody else's database, and it currently ends
      by reassuring the reader.
- [x] **T033** FR-026 / SC-006 — `README.md`: "Taking over a database the gateway already migrated"
...
- [x] **T034** Run the authoritative command and confirm green with the store tests **still
...
- [x] **T035** `ruff check` clean (the repo configures `E`, `F`, `I`, `N`, `W` at line-length 100).
- [x] **T036** Re-read `tests/conftest.py`'s `OPTIONAL_DRIVERS` disclosure: the new tests gate
