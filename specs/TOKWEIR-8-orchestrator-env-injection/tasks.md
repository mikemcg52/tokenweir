...
- [ ] **T001** Create `src/tokenweir/orchestrator.py` with the module docstring: what the
...
- [ ] **T002** Define `PhaseKind` (`implementation`, `review`, `fix`) as a `str` Enum with a
  tolerant `coerce`, mirroring `PricingMode` (FR-001).
- [ ] **T003** Implement `phase_label(kind, occurrence=None)` — canonical `kind` / `kind-N`,
  raising on a non-positive or non-integral occurrence and on an unknown kind (FR-002, FR-007).
- [ ] **T004** Implement the parser behind `normalize_phase`: separator folding (`_`, `#`,
...
- [ ] **T005** Implement `normalize_phase(value)` — `None`/blank → `None` (FR-006); recognized →
...
- [ ] **T006** Define `ATTRIBUTION_ENV` as the single spelling of the four variable names
...
- [ ] **T007** Export the new public names from `src/tokenweir/__init__.py` `__all__`.

## Phase 2 — The consumer side

- [ ] **T008** In `src/tokenweir/claude_code.py`, route `MADO_PHASE` through `normalize_phase` in
...
- [ ] **T009** [P] `tests/test_orchestrator.py`: taxonomy and normalization tables — the SC-001
...
- [ ] **T010** [P] `tests/test_orchestrator.py`: `phase_label` and `attribution_env` producer
...
- [ ] **T011** [P] `tests/test_orchestrator.py`: SC-006 — the module imports nothing outside the
  standard library and does not import `tokenweir.claude_code`.
- [ ] **T012** Round-trip test (SC-003, the story's acceptance): build a block for a given issue
...
- [ ] **T013** Hook tests: a non-canonical `MADO_PHASE` reaches the record canonical; an
  unrecognized one reaches the record intact and is noted (SC-004); existing hook expectations
  unchanged.

## Phase 4 — Documentation

- [ ] **T014** README: extend the attribution section with the taxonomy (kinds, label grammar,
...
- [ ] **T015** Run the authoritative suite: `CI=true .venv/bin/pytest` from `/workspace/repo`, per
